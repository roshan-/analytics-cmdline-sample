...
public class ManagementApiReferenceSample {
  ...
  private static void uploadData(Analytics analytics) throws IOException {
    File file= new File("data.csv");
    InputStreamContent data= new InputStreamContent("application/octet-stream", new FileInputStream(file));
...
//      Upload uQA= analytics.management().uploads().uploadData("96667371", "UA-53906132-4", "GkcrBboGSEas_qNZvBt58w", data).execute();
      Upload u3936= analytics.management().uploads().uploadData("96667371", "UA-53906132-5", "H2OefjqiRIavtH5mOl5XYQ", data).execute();
    } catch (GoogleJsonResponseException e) {
  ...
  private static Credential authorize() throws Exception {
    ...
    GoogleAuthorizationCodeFlow flow = new GoogleAuthorizationCodeFlow.Builder(
        ...
        Collections.singleton(AnalyticsScopes.ANALYTICS)).setDataStoreFactory(
